...
void buffer_set_cell_styled(Buffer *buf, int row, int col, char ch,
...
static int color_eq(Color a, Color b) {
    ...
    switch (a.type) {
        case COLOR_DEFAULT: return 1;
        case COLOR_INDEXED: return a.r == b.r;  // 'r' stores the index
...
static int write_color_escape(char *out, Color color, int is_fg) {
    ...
    switch (color.type) {
        case COLOR_DEFAULT:
            ...
            break;
        case COLOR_INDEXED:
            pos += sprintf(out + pos, "\x1b[%d;5;%dm", base, color.r);  // 'r' stores the index
            break;
        case COLOR_RGB:
            pos += sprintf(out + pos, "\x1b[%d;2;%d;%d;%dm",
...
static int write_attr_escape(char *out, uint8_t attrs) {
    int pos = 0;
    if (attrs & ATTR_BOLD)      pos += sprintf(out + pos, "\x1b[1m");
    if (attrs & ATTR_DIM)       pos += sprintf(out + pos, "\x1b[2m");
    if (attrs & ATTR_ITALIC)    pos += sprintf(out + pos, "\x1b[3m");
    if (attrs & ATTR_UNDERLINE) pos += sprintf(out + pos, "\x1b[4m");
    if (attrs & ATTR_BLINK)     pos += sprintf(out + pos, "\x1b[5m");
    if (attrs & ATTR_REVERSE)   pos += sprintf(out + pos, "\x1b[7m");
    if (attrs & ATTR_HIDDEN)    pos += sprintf(out + pos, "\x1b[8m");
    if (attrs & ATTR_STRIKE)    pos += sprintf(out + pos, "\x1b[9m");
    return pos;
}
