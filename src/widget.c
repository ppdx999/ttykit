...
Widget *widget_list(Constraint c, const char **items, size_t count, size_t selected) {
    Widget *w = arena_alloc(&g_frame_arena, sizeof(Widget));
    if (!w) return NULL;
...
static void render_list(Buffer *buf, Rect area, const char **items, size_t count, size_t selected);

void widget_render(Widget *w, Buffer *buf, Rect area) {
    if (!w || rect_is_empty(area)) return;

    switch (w->type) {
        case WIDGET_VBOX:
        case WIDGET_HBOX: {
            Direction dir = (w->type == WIDGET_VBOX)
                          ? DIRECTION_VERTICAL
            ...
            Constraint *constraints = arena_alloc(&g_frame_arena, sizeof(Constraint) * n);
            Rect *areas = arena_alloc(&g_frame_arena, sizeof(Rect) * n);
            if (!constraints || !areas) return;
...
static void render_block(Buffer *buf, Rect area, const char *title, Widget *child) {
    if (rect_is_empty(area)) return;

    Color border_color = COLOR_INDEX(8);  // Gray

    // Draw top border
    buffer_set_cell_styled(buf, area.y, area.x, '+', border_color, COLOR_DEFAULT_INIT, ATTR_NONE);
    for (uint16_t c = area.x + 1; c < area.x + area.width - 1; c++) {
        buffer_set_cell_styled(buf, area.y, c, '-', border_color, COLOR_DEFAULT_INIT, ATTR_NONE);
    }
    if (area.width > 1) {
        buffer_set_cell_styled(buf, area.y, area.x + area.width - 1, '+', border_color, COLOR_DEFAULT_INIT, ATTR_NONE);
    }
    ...
    if (title && area.width > 4) {
        ...
        buffer_set_cell_styled(buf, area.y, area.x + 1, ' ', COLOR_DEFAULT_INIT, COLOR_DEFAULT_INIT, ATTR_NONE);
        for (size_t i = 0; i < title_len; i++) {
            buffer_set_cell_styled(buf, area.y, area.x + 2 + i, title[i],
                                   COLOR_INDEX(11), COLOR_DEFAULT_INIT, ATTR_BOLD);
        }
        buffer_set_cell_styled(buf, area.y, area.x + 2 + title_len, ' ', COLOR_DEFAULT_INIT, COLOR_DEFAULT_INIT, ATTR_NONE);
    }

    // Draw side borders
    for (uint16_t r = area.y + 1; r < area.y + area.height - 1; r++) {
        buffer_set_cell_styled(buf, r, area.x, '|', border_color, COLOR_DEFAULT_INIT, ATTR_NONE);
        if (area.width > 1) {
            buffer_set_cell_styled(buf, r, area.x + area.width - 1, '|', border_color, COLOR_DEFAULT_INIT, ATTR_NONE);
        }
    }
    ...
    if (area.height > 1) {
        ...
        buffer_set_cell_styled(buf, bottom, area.x, '+', border_color, COLOR_DEFAULT_INIT, ATTR_NONE);
        for (uint16_t c = area.x + 1; c < area.x + area.width - 1; c++) {
            buffer_set_cell_styled(buf, bottom, c, '-', border_color, COLOR_DEFAULT_INIT, ATTR_NONE);
        }
        if (area.width > 1) {
            buffer_set_cell_styled(buf, bottom, area.x + area.width - 1, '+', border_color, COLOR_DEFAULT_INIT, ATTR_NONE);
        }
    }
...
static void render_list(Buffer *buf, Rect area, const char **items, size_t count, size_t selected) {
    if (rect_is_empty(area) || !items) return;
