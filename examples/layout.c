...
static void draw_centered(Buffer *buf, Rect r, const char *text, Color fg,
                          uint8_t attrs) {
  int len = strlen(text);
...
static void draw_screen(Buffer *buf, int rows, int cols, const char *status) {
  ...
  // Draw content
  draw_centered(buf, content, "Hello, ttykit!", COLOR_RGB_INIT(0, 255, 255),
                ATTR_BOLD);

  // Demo attributes in content area
  int demo_row = content.y + content.height / 2 + 2;
  int demo_col = content.x + 2;
  buffer_set_str_styled(buf, demo_row, demo_col, "Bold", COLOR_INDEX(1),
                        COLOR_DEFAULT_INIT, ATTR_BOLD);
  buffer_set_str_styled(buf, demo_row, demo_col + 6, "Italic", COLOR_INDEX(3),
                        COLOR_DEFAULT_INIT, ATTR_ITALIC);
  buffer_set_str_styled(buf, demo_row, demo_col + 14, "Underline",
                        COLOR_INDEX(4), COLOR_DEFAULT_INIT, ATTR_UNDERLINE);
  buffer_set_str_styled(buf, demo_row, demo_col + 25, "Reverse", COLOR_INDEX(5),
                        COLOR_DEFAULT_INIT, ATTR_REVERSE);

  // Draw footer line
  draw_hline(buf, footer, '-', COLOR_INDEX(2));
  buffer_set_str_styled(buf, footer.y, footer.x + 1, status, COLOR_INDEX(11),
