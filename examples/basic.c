...
static void draw_screen(Buffer *buf, int rows, int cols, const char *status) {
  ...
  buffer_set_str_styled(buf, rows / 2, (cols - len) / 2, msg,
                        COLOR_RGB_INIT(0, 255, 255), COLOR_DEFAULT_INIT,
                        ATTR_BOLD);

  // Draw border with 256-color (green)
  Color green = COLOR_INDEX(2);
  for (int c = 0; c < cols; c++) {
    buffer_set_cell_styled(buf, 0, c, '-', green, COLOR_DEFAULT_INIT,
                           ATTR_NONE);
    buffer_set_cell_styled(buf, rows - 1, c, '-', green, COLOR_DEFAULT_INIT,
                           ATTR_NONE);
  }
  for (int r = 0; r < rows; r++) {
    buffer_set_cell_styled(buf, r, 0, '|', green, COLOR_DEFAULT_INIT,
                           ATTR_NONE);
    buffer_set_cell_styled(buf, r, cols - 1, '|', green, COLOR_DEFAULT_INIT,
                           ATTR_NONE);
  }

  // Demo different attributes
  int demo_row = rows / 2 + 2;
  buffer_set_str_styled(buf, demo_row, 2, "Bold", COLOR_INDEX(1),
                        COLOR_DEFAULT_INIT, ATTR_BOLD);
  buffer_set_str_styled(buf, demo_row, 8, "Italic", COLOR_INDEX(3),
                        COLOR_DEFAULT_INIT, ATTR_ITALIC);
  buffer_set_str_styled(buf, demo_row, 16, "Underline", COLOR_INDEX(4),
                        COLOR_DEFAULT_INIT, ATTR_UNDERLINE);
  buffer_set_str_styled(buf, demo_row, 27, "Reverse", COLOR_INDEX(5),
                        COLOR_DEFAULT_INIT, ATTR_REVERSE);

  // Show status at bottom
  buffer_set_str_styled(buf, rows - 1, 2, status, COLOR_INDEX(11),
